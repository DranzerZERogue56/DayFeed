...
// throws TranscriptionBusyError on a second. Two features want to
// transcribe — the manual ✎ Transcribe button and voice dictation — and
// the two overlapping used to surface as a "Please wait" alert for
// something the user often never asked for.
//
 ...
 *
 * The returned promise settles with the job's own result — so callers await
 * it and surface a failure. A rejected job does not poison the queue for the
 * jobs behind it.
 */
